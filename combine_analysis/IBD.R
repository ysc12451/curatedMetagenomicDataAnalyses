...
normalized_otu_data = top_300_PCs #[1260,300]
# Extract sample metadata
sample_data <- colData(tse)

sample_id = which(sample_data$disease %in% c('healthy', 'IBD'))
...
response_train_binary <- ifelse(response_train == "IBD", 1, 0)
...
predictions <- ifelse(predictions_prob > 0.5, "IBD", "healthy")

# Confusion matrix
response_test <- ifelse(response_test == "IBD", "IBD", "healthy")
...
dim(normalized_otu_data)
normalized_otu_data = normalized_otu_data[1:1260,]
dim(normalized_otu_data)
